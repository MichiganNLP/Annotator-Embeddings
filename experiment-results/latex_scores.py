...
def process_dataset(dataset, settings, score_type, line_colors, last_row):
    ...
    for i, line in enumerate(latex_ss):
        ...
        if i == len(latex_ss) - 1:
            prefix += last_row
            # c_latex_ss.append(prefix + f' & \\cw\\multirow{{-6}}{{*}}{{{round(100 *mean(scores["random"]["random"]), 2):.2f}}} & \\cw\\multirow{{-6}}{{*}}{{{round(100 *mean(scores["individual_majority_vote"]["individual_majority_vote"]), 2):.2f}}} & \\cw\\multirow{{-6}}{{*}}{{{round(100 *mean(scores["overall_majority_vote"]["overall_majority_vote"]), 2):.2f}}} & ' + line)
            c_latex_ss.append(prefix + f' & \\cw\\multirow{{-6}}{{*}}{{{round(100 *mean(scores["random"]["random"]), 2):.2f}}}  & \\cw\\multirow{{-6}}{{*}}{{{round(100 *mean(scores["overall_majority_vote"]["overall_majority_vote"]), 2):.2f}}} & ' + line)
        else:
            c_latex_ss.append(prefix + " & \\cw & \\cw & \\cw & " + line)
    print("\n".join(c_latex_ss))
    print("\n")
...
def main():
    ...
    datasets = ["friends_qia"]
    # settings = ["bert-base", "bert-base-naiive-concat", "random", "individual_majority_vote", "overall_majority_vote", \
    settings = ["bert-base", "bert-base-naiive-concat", "random", "overall_majority_vote"]
                # "bert-large", "bert-large-naiive-concat",\
                # "roberta-base", "roberta-base-naiive-concat",\
                #      "roberta-large", "roberta-large-naiive-concat",\
                #     "deberta-base", "deberta-base-naiive-concat",\
                #         "deberta-large", "deberta-large-naiive-concat"]  # wo_embed corresponds to the bert-base ones
