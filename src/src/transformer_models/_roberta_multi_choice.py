...
class CustomizedRobertaEmbeddings(nn.Module):
    """
        ...
        if self.method == "add":
            ...
            if annotator_ids is not None:
                ...
                if self.broadcast_annotator_embedding:
                    embeddings = embeddings + annotator_embed_after_alpha.unsqueeze(1).repeat(1, K, 1)
                else:
            ...
            if annotations is not None:
                ...
                if self.include_pad_annotation:
                    ...
                    if self.broadcast_annotation_embedding:
                        embeddings = embeddings + annotation_embed_after_beta.unsqueeze(1).repeat(1, K, 1)
                    else:
...
class RobertaMultiChoice(RobertaModel):

    def __init__(self, config: RobertaConfig, tasks: Tasks, decoder_tokenizers: dict,
        ...
        self.embeddings = CustomizedRobertaEmbeddings(config, 
