    ...
    def test(self, seed, idx):
        ...
        for itm in self.test_data:
            with open(itm["path"], 'r') as f:
                ...
                pred_label = most_frequent(self.train_preference[respondent_id])
                predictions.append({
                    "pred": pred_label,
                    "gold": gold_label,
        ...
        with open(f"{self.output_path}/use_annotator_embed-False-use_annotation_embed-False-pad-True-method-add-test_mode-normal-seed-{seed}-{idx}-annotation-0.jsonl", 'w') as f:
...
if __name__ == "__main__":
    ...
    for dataset in ["toxic-ratings"]:
        ...
        output_path = f"../experiment-results/{dataset}/individual_majority_vote"
        imv = IndividualMajorityVote(task, train_path, test_path, output_path)
        imv.train()
