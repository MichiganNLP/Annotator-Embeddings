...
#SBATCH --output=/home/dnaihao/dnaihao-scratch/annotator-embedding/experiment-results/sentiment/deberta-large-naiive-concat/%x-%j.log
#SBATCH --job-name=sentiment-deberta-v3-large
...
model_name=deberta-multichoice
dataset=sentiment
tasks=sentiment
...
out_model_name_or_path=deberta-large
...
do  
    ...
    do  
        ...
        do  
            ...
            echo $output_fn
            pred_fn_path=../experiment-results/$dataset/deberta-large-naiive-concat/$output_fn.jsonl
        ...
        do  
            ...
            output_fn=use_annotator_embed-${use_annotator_embed}-use_annotation_embed-${use_annotation_embed}-pad-${include_pad_annotation}-method-${method}-test_mode-${test_mode}-seed-$seed-$i-$split_method-$tt_idx                
            echo $output_fn
            pred_fn_path=../experiment-results/$dataset/$out_model_name_or_path/$output_fn.jsonl
            if [ -e "$pred_fn_path" ]; then
                echo "File exists: $pred_fn_path"
            else
                python -m src ${model_name} \
